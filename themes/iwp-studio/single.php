<?php
/**
 * Single post. Dark title block + prose + auto-built sticky TOC + author bio +
 * share + related + CTA. Styles in assets/blog.css; behaviour in assets/post.js.
 */
...
while ( have_posts() ) :
	...
	$read      = iwps_blog_read_time( $post_id );

	list( $content_html, $toc ) = iwps_blog_toc( apply_filters( 'the_content', get_the_content() ) );
	?>
...
<article>
	<header class="post-hero">
		...
		<div class="wrap">
			...
			<?php if ( $cats ) : ?>
				<div class="cat-row">
					<?php foreach ( array_slice( $cats, 0, 2 ) as $c ) :
						$cs = iwps_blog_term_style( $c ); ?>
						<a class="cat<?php echo $cs['pill'] ? ' ' . esc_attr( $cs['pill'] ) : ''; ?>" href="<?php echo esc_url( get_category_link( $c ) ); ?>"><span><?php echo esc_html( $c->name ); ?></span></a>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
			<nav class="crumb" aria-label="Breadcrumb">
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>">Home</a><span class="sep">/</span>
				<?php $blog = (int) get_option( 'page_for_posts' ); if ( $blog ) : ?>
					<a href="<?php echo esc_url( get_permalink( $blog ) ); ?>">Blog</a><span class="sep">/</span>
				<?php endif; ?>
				<?php if ( $cats ) : ?>
			...
			<div class="meta">
				<div class="author">
					<span class="avatar"><?php echo esc_html( iwps_blog_initials( $author ) ); ?></span>
					<div>
						<div class="a-name"><?php echo esc_html( $author ); ?></div>
						<?php if ( $role ) : ?><div class="a-role"><?php echo esc_html( $role ); ?></div><?php endif; ?>
					</div>
				</div>
	...
	<div class="post-foot">
		<div class="bio">
			...
			<div class="bio-bd">
				...
				<?php endif; ?>
				<div class="bio-links">
					<a href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>">More posts</a>
				</div>
			</div>
		</div>
		...
		<?php
		$related = iwps_blog_related( $post_id, 3 );
		if ( $related ) :
			...
			<section class="related">
				...
				<div class="rel-grid">
					<?php foreach ( $related as $i => $rp ) :
						$rmeta = iwps_blog_cat_meta( $rp->ID ); ?>
						<a class="rcard" href="<?php echo esc_url( get_permalink( $rp ) ); ?>">
							<div class="thumb <?php echo esc_attr( $rth[ $i % 3 ] ); ?>"><span class="mk"></span><span class="gl"></span><span class="tcat"><?php echo esc_html( $rmeta['label'] ); ?></span><?php $rimg = iwps_blog_card_img( $rp ); if ( $rimg ) { echo '<img class="pc-thumb-img" src="' . esc_url( $rimg ) . '" alt="' . esc_attr( get_the_title( $rp ) ) . '" loading="lazy">'; } ?></div>
							<div class="rc-bd">
								<h3><?php echo esc_html( get_the_title( $rp ) ); ?></h3>
								<div class="rc-meta"><span><?php echo esc_html( get_the_author_meta( 'display_name', $rp->post_author ) ); ?></span><span class="sep">&middot;</span><span><?php echo esc_html( iwps_blog_read_time( $rp ) ); ?></span></div>
							</div>
						</a>
					<?php endforeach; ?>
				</div>
