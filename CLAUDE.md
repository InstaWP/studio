...
the `iwp-studio` plugin renders them live as real WordPress pages (it rides on any
theme). **No page builder, no block editor, no build step, no DB-stored content.**
You (the AI agent) author and edit the HTML; WordPress serves, routes, and lets a
human edit it.
...
- Each `.html` in `site/` is a page. The plugin's page map (`instawp_homebuild_pages()`)
...
- Body starts with `<div id="site-nav"></div>` and ends with `<div id="site-footer"></div>`
  — the renderer keeps only what's between them (`assets/chrome.js` fills the real nav/footer).
...
- **Ship** — publish to InstaWP (`SITE=<sandbox> bash scripts/publish.sh`), only when
  asked. Run WP-CLI on a cloud site with `SITE=<sandbox> bash scripts/wp.sh <cmd>` (both
  wrap the `instawp` CLI and fall back around the chroot bug, see
